...
Le soir, le LLM contre Meander finit : 1000 parties, 1058 minutes, 0 réponse invalide. Moyenne -36,11, IC 95 % [-37,14 ; -35,08]. E1 a ses cinq lignes.

Le chiffre que je ne voyais pas venir : contre B_line, le LLM est à -33,71 du hasard ; contre Meander, à -1,51. Vingt-deux fois moins. Mon « le LLM fait pire que le hasard » n'est donc pas une propriété du LLM, c'est une propriété du couple LLM-attaquant. Contre celui qui fonce, il s'effondre ; contre celui qui traîne, il tient presque. L'hypothèse à tester dans les logs : il met trop de tours à converger sur la bonne action, ce qui ne pardonne pas face à une ruée directe.

Et l'apparié hasard vs LLM sur Meander donne +1,51 avec un IC à [+0,06 ; +2,96]. La borne haute est à six centièmes de zéro : c'est significatif et ça ne veut rien dire. Sur un terrain où les scores vont de -12 à -130, un point et demi n'a aucune portée opérationnelle, et pourtant mille graines le détectent. C'est mon propre protocole qui montre sa limite : la significativité statistique n'est pas la significativité pratique. Je n'ai plus besoin d'un exemple d'école pour l'illustrer.

Trouvé aussi aujourd'hui, et ça aurait dû l'être le premier jour : le classement officiel de CAGE 2 est dans le README du dépôt que j'ai cloné. Mon hasard fait -154,71 contre -154,06 pour leur agent aléatoire, et -34,60 contre -33,43 sur Meander. Mon PPO fait -4,70 quand leur PPO simple publié fait -4,76 et que le vainqueur fait -3,47. Quatre points de référence reproduits sans le savoir. Et en lisant leur evaluation.py, je retrouve mon propre cage2_campagne.py : même ChallengeWrapper, même construction du chemin Scenario2.yaml, même appel CybORG. J'ai reconstruit leur banc de mesure sans jamais l'avoir ouvert. La leçon n'est pas « j'ai sous-investi dans la bibliographie », c'est « j'ai construit sur un outil sans lire sa documentation ».

Deux corrections de dépôt dans la foulée. analyse_profondeur.py, le script qui produit le tableau 4.3, n'était pas dans preuves/ et chargeait deux CSV qui n'existent plus (cage2_llm.csv, resultats_llm_v2_nuit.csv). Réparé, copié dans 01_scripts/, relancé : il retrouve 69,3 / 7,7 / 23,0 et 0,2 / 57,9 / 41,9. Le tableau 4.3 est reproductible, il ne l'était pas.

Enfin j'ai relancé le PoC AICA-en-LangGraph de mai, qui n'apparaît nulle part dans le mémoire. Le venv du dossier n'existait plus (un venv contient des chemins absolus, il ne survit pas à un déplacement) et je tournais sans m'en rendre compte avec celui de CybORG : quatre ModuleNotFoundError. Même piège que le shadowing de CybORG, en costume différent — ce n'est pas le dossier qui décide cette fois, c'est le venv actif. Venv refait, dépendances réinstallées : pip a tiré langgraph 1.2.9 là où mon requirements.txt disait >=0.2.50, donc un saut de version majeure, plus Python 3.14. Le code de mai tourne sans une ligne à changer. La chaîne complète sensing -> perception -> world_model -> decision -> action s'exécute, l'agent isole le pod et coupe la session PDU en trois pas. Versions figées dans requirements-lock.txt.

Une chose vue en lisant la sortie, et qui vaut mieux qu'un test : mon agent isole open5gs-smf-0, pas attacker-pod. Mes trois événements mock comptent deux Warning sur le SMF et un événement de type Normal sur l'attaquant ; perception_node ne garde que les Warning, donc l'attaquant n'atteint jamais le modèle du monde. L'agent isole la victime et laisse l'attaquant tranquille — et isoler le SMF d'un cœur 5G, c'est réaliser soi-même le déni de service que l'attaquant cherchait. Sur CAGE 2, restaurer l'hôte compromis est juste parce que l'hôte compromis est le problème. En cloud-native, l'hôte qui alerte est souvent la victime, et l'isoler est l'attaque. La sémantique des actions ne se transporte pas d'un terrain à l'autre.

## 18 juillet

La campagne LLM 145 actions a fini dans la nuit : 1000 parties, 1901 minutes, 0 invalide. Moyenne -176,06.

Nos deux prédictions sont fausses, et dans le même sens. J'avais dit -200 et 0 % d'invalides, Claude -200 à -230 et 5-20 % d'invalides. On était persuadés tous les deux que rendre 145 actions au lieu de 41 le dégraderait. C'est l'inverse : il gagne 12 points (apparié +12,36 [+5,88 ; +18,85], significatif). Sur les invalides j'avais raison, 0 : la fenêtre d'acceptation s'élargit quand le menu s'allonge, elle ne peut que baisser.

Mais la moyenne ment. Il ne gagne que 333 parties sur 1000 contre le v1 : sur les deux tiers des scénarios il fait pareil ou pire. Sa variance passe de 54 à 84. L'amélioration vient de quelques gros coups, pas d'un meilleur jeu.

Le vrai résultat est dans les actions, que le script a journalisées cette fois. Sur 145 actions il en utilise 16. Il joue Restore Enterprise0 56 % du temps (16 808 coups sur 30 000), Monitor 26,6 %, et zéro leurre sur 30 000 coups. Donné un grand espace d'action, il ne l'explore pas : il s'effondre sur une quasi-action unique. C'est exactement ce que fait le PPO sur DroneSwarm au contrôle 4 (une action 900/900). Un LLM et un réseau de neurones, deux technologies opposées, font la même chose face à un grand espace d'action. Seule ma règle écrite à la main s'en sert vraiment. Ça relie le PoC (le LLM qui isole toujours la victime), le contrôle 4 et le chapitre LLM sous une seule idée.
